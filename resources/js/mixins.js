    ...
    methods: {
     ...
     // convert dates to moment object
        ...
        sendEmail() {
            ...
            //show loader until send notification
            this.showLoaderNotification(trans('em.booking_success')+' '+trans('em.booking_mail'));

            axios.get(route('eventmie.send_email'))
            .then(res => {
                if(res.status > 0){
                    
                    // hide loader
                    Swal.hideLoading();
                    let msg = '<p>'+trans('em.booking_success')+'</p>';
                    msg     += '<p>'+trans('em.booking_email_sent')+'</p>';
